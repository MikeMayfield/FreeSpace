...
class FtpManager {
     ...
     *
     * @param fullRemoteFilePath The full path of the file on the FTP server (including leading "/" for root directory)
     ...
     * @throws IllegalStateException If not connected to the FTP server.
     */
    fun downloadFile(fullRemoteFilePath: String, localFile: File): Boolean {
        if (!isConnected()) {
        ...
        try {
            FileOutputStream(localFile).use { outputStream: OutputStream ->
                return ftpClient.retrieveFile(fullRemoteFilePath, outputStream)
            }
        } catch (e: IOException) {
            Log.e("downloadFile", "Error downloading file '$fullRemoteFilePath' from FTP server: ${e.message}")
            if (localFile.exists()) {
                localFile.delete()
            }
            return false
    ...
    fun renameRemoteFile(fromPath: String, newName: String): Boolean {
        ...
        try {
            if (changeToWorkingDirectory(extractPathFromFullPath(fromPath))) {
                val fromName = fromPath.substringAfterLast('/')

...
class PeriodicBackgroundProcessingWorker(val appContext: Context, val params: WorkerParameters): CoroutineWorker(appContext, params) {
    //TODO Refactor to allow background processing chain to be started only if idle
    override suspend fun doWork(): Result {
        val result = withContext(Dispatchers.IO) {
            ...
            if (bytesToRecover > 0) {
                ...
                addAllNewMediaFilesToDB(appContext, mediaFileRepository)  //Add all new media files to DB
                updateDesiredCompressionLevelsInDB(mediaFileRepository)  //Update potential compression level for all files

                queueWorkerToProcessFirstFile(appContext)  //Queue SelectFileToCompress worker chain for first file to be processed (will requeue itself for each additional file needed)
            }
    ...
    private fun addAllNewMediaFilesToDB(context: Context, mediaFileRepository: MediaFileRepository) {
     ...
     */
    private fun queueWorkerToProcessFirstFile(context: Context) {
    ...
    companion object {
        ...
        fun queuePeriodicProcessing(context: Context, periodHours: Long) {
            ...
            WorkManager.getInstance(context).enqueueUniqueWork(
                ...
                ExistingWorkPolicy.KEEP,  //Don't queue if already running or queued
                request
            )
